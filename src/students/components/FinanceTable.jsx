...
const FinanceTable = () => {
  const [studentData] = useGlobalState("student");
  ...
  // const getStudent = () => {
  //   return student.slice(0, end)
  // }

  useEffect(() => {
    // setAllStudents(getStudent())
    console.log("student at frontend", studentData)
  }, [studentData, end])
  ...
  return (
    <>
      ...
      <div className="mb-4">
        <h1 className="text-3xl font-bold text-gray-800 mb-2 dark:text-gray-400">Student Financial Records</h1>
      </div>
          ...
          <table className="min-w-full overflow-x-auto  bg-white border-b border-gray-700 dark:bg-[#212936] dark:text-gray-300 dark:border-gray-700">
            <thead className='bg-gray-50 border-b-2 border-gray-200 dark:bg-gray-900 dark:gray-300'>
              ...
              <th className="py-2 px-4 text-start text-lg">ID</th>
                 ...
                 <th className="py-2 px-4 text-start text-lg uppercase">School Fee</th>
                 <th className="py-2 px-4 text-start text-lg uppercase">Accomodation Fee</th>
                 <th className="py-2 px-4 text-start text-lg uppercase">School Fee Paid Amount</th>
                 <th className="py-2 px-4 text-start text-lg uppercase">Accomodation Fee Paid Amount</th>
            ...
            <tbody>
              {studentData.map((student, index) => (
                ...
                >
                    ...
                    <td className={`py-2 px-4 text-center text-gray-700 text-base border-b dark:text-gray-500 ${hoveredRow === index ? 'bg-gray-200 dark:bg-gray-900' : ''}`}>{student.schoolFee.toString()}</td>
                    <td className={`py-2 px-4 text-center text-gray-700 text-base border-b dark:text-gray-500 ${hoveredRow === index ? 'bg-gray-200 dark:bg-gray-900' : ''}`}>{student.accomodationFee.toString()}</td>
