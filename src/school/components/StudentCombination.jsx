...
const StudentCombinationTable = () => {
  const [studentCombinations] = useGlobalState("studentCombinations");
  const [hoveredRow, setHoveredRow] = useState(null);
  const [allStudentCombination, setAllStudentCombination] = useState([])
  const [end, setEnd] = useState(6)

  console.log("allStudentCombination: ", allStudentCombination)
  ...
  const getStudentCombinations = () => {
    return studentCombinations.slice(0, end)
  }

  useEffect(() => {
    setAllStudentCombination(getStudentCombinations())
    console.log(studentCombinations)
  }, [studentCombinations, end])
  ...
  return (
    <>
      ...
      <div className="mb-4">
        <h1 className="text-3xl font-bold text-gray-800 mb-2 dark:text-gray-400">Student Combination Lists</h1>
      </div>
          ...
          <table className="min-w-full overflow-x-auto  bg-white border-b border-gray-700 dark:bg-[#212936] dark:text-gray-300 dark:border-gray-700">
            <thead className='bg-gray-50 border-b-2 border-gray-200 dark:bg-gray-900 dark:gray-300'>
              <tr className='border-none'>
                ...
                <th className="py-2 px-4 border-b text-center text-lg uppercase">Combination Name</th>
            ...
            <tbody>
              {allStudentCombination.map((combination, index) => (
                <tr
                  key={index}
                ...
                >
                  ...
                  <td className={`py-2 px-4 text-center text-gray-700 text-base border-b dark:text-gray-500 ${hoveredRow === index ? 'bg-gray-200 dark:bg-gray-900' : ''}`}>{combination.combinationName}</td>

                  <td className={`w-20 py-2 px-4 text-gray-700 text-base border-b ${hoveredRow === index ? 'bg-gray-200 dark:bg-gray-900' : ''}`}><button onClick={() => deleteStudentHandler(combination.combinationId)} className='border border-solid bg-red-400 hover:bg-red-500 active:bg-red-400 px-3 py-1 border-r-2 text-white dark:bg-transparent dark:text-gray-500 gap-1 flex items-center dark:border-red-500'><BiEdit size={17} />Edit</button></td>
...
export default StudentCombinationTable;